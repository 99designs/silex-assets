...
class StylesheetNode extends \Twig_Node
{
    ...
    public function compile(\Twig_Compiler $compiler)
    {
        ...
        foreach($this->_files as $file) {
            $webPath = $this->_config['css_web_path'] . '/' . $file;
            $filePath = realpath($this->_config['css_output_dir'] . '/' . $file);

            if(!is_file($filePath)) {
                throw new \Exception("Failed to find ".
                    ($this->_config['css_output_dir'] . '/' . $file));
            }
            ...
            $compiler
                ->write(sprintf(
                    'echo "<link rel=\"stylesheet\" href=\"%s%s\" />"', $webPath, $buster
                ))
                ->raw(";\n")
            ;
        }
